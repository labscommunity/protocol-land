...
export default function BlogCard({ data, createBlogClickHandler }: Props) {
  return (
    <div
      onClick={createBlogClickHandler(data)}
      className="group hover:cursor-pointer h-full w-full flex flex-col gap-10 p-[40px] rounded-2xl border border-sky-300 border-opacity-20 bg-[linear-gradient(180deg,rgba(56,124,158,0.20)_0%,rgba(0,0,0,0.20)_100%)]"
    >
      <div className="group-hover:opacity-[0.6] transition-opacity ease-in duration-400 rounded-[15px] overflow-hidden w-full relative">
        <img src={data.coverImage} alt="" className="object-cover object-center w-full h-full" />
      </div>
