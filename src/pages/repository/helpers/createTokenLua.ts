...
export function createImportedTokenLua(token: RepoToken, balances: Record<string, string>): string {
  let luaCode = `
-- module: "src.utils.mod"
local function _loaded_mod_src_utils_mod()
  local bint = require('.bint')(256)
...
local function _loaded_mod_src_handlers_token()
  ...
  Denomination = Denomination or ${+token.denomination}
  --- @type Balances
  Balances = Balances or json.decode('${JSON.stringify(balances)}')
  --- @type TotalSupply
  TotalSupply = TotalSupply or utils.toBalanceValue('${preventScientificNotationFloat(+token.totalSupply * 10 ** +token.denomination)}')
  
  if Name ~= '${token.tokenName}' then
